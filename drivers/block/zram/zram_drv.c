...
static int mem_used(int id, void *ptr, void *data)
{
	struct zram *zram = ptr;
	if(!zram || !init_done(zram) || !zram->disk)
		return 0;

	if (zram && zram->mem_pool)
		*((unsigned long *)data) += zs_get_total_pages(zram->mem_pool);
	return 0;
}
...
static ssize_t mem_free_percent(void)
{
	...
	idr_for_each(&zram_index_idr, &mem_used, &mem_used_pages);
...
static struct class_attribute zram_control_class_attrs[] = {
	...
	/* __ATTR_WO does not yet exist in 3.10 */
	{							\
	.attr	= { .name = "hot_remove", .mode = S_IWUSR },	\
	.store	= hot_remove_store,				\
	},
	__ATTR_NULL,
};
